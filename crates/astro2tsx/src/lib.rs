...
pub struct ConvertResult {
    ...
    /// Source position for the generated frontmatter terminator mapping, if emitted.
    pub(crate) frontmatter_terminator_anchor: Option<u32>,
    /// Generated byte range covering the printed frontmatter section.
    pub frontmatter_range: GeneratedRange,
    /// Generated-only completion slot when the source has no frontmatter.
...
pub fn convert_to_tsx(source: &str, options: ConvertOptions) -> ConvertResult {
    ...
    ConvertResult {
        ...
        frontmatter_terminator_anchor,
        frontmatter_range: printer.frontmatter_range,
        frontmatter_insertion_range: printer.frontmatter_insertion_range,
...
mod tests {
    ...
    use crate::{ConvertOptions, FrontmatterStatus, convert_to_tsx};
    ...
    #[test]
    fn missing_frontmatter_records_a_generated_completion_slot() {
        let source = "<Ima";
        let result = convert_to_tsx(source, ConvertOptions::default());
        assert_eq!(result.frontmatter.status, FrontmatterStatus::DoesntExist);
        assert_eq!(result.frontmatter_range.start, result.frontmatter_range.end);

        let insertion = result.frontmatter_insertion_range.unwrap();
        assert_eq!(insertion.end, result.frontmatter_range.start);
        assert_eq!(
            &result.code[insertion.start as usize..insertion.end as usize],
            "\n"
        );
        assert!(result.mappings.iter().any(|mapping| {
            mapping.generated == result.body.start && mapping.original == Some(0)
        }));

        let with_frontmatter = convert_to_tsx("---\n---\n\n<Ima", ConvertOptions::default());
        assert_eq!(
            with_frontmatter.frontmatter.status,
            FrontmatterStatus::Closed
        );
        assert!(with_frontmatter.frontmatter_insertion_range.is_none());
    }

    #[test]
    fn frontmatter_terminator_is_mapped_to_the_closing_fence() {
        let source = "---\nimport Hello from './Hello.astro'\nimport Layout from './Layout.astro'\n---\n<Layout><Hello /></Layout>";
        let result = convert_to_tsx(source, ConvertOptions::default());
        assert!(!result.has_parse_errors, "{:?}", result.diagnostics);

        let anchor = result.frontmatter_terminator_anchor.unwrap();
        assert_eq!(
            &result.code[result.frontmatter_range.end as usize..result.body.start as usize],
            ";{};<Fragment>\n"
        );
        assert_eq!(anchor as usize, source.rfind("---").unwrap());
    }
